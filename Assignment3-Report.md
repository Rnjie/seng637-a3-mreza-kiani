...
In this assignment, we want to apply white-box testing for the JFreeChart library. We want to work with coverage metrics, as
...
In the end, we could reach high coverage numbers for these 2 classes which were beyond our goal.
...
For reaching more than 90% in line coverage, we wanted to do 2 things:
...
below, shows an example of a previously covered method by black-box testing:
...
By extracting the available branch for each method, and trying to make the condition both true and false, we can go
...
have branch and condition coverages. After searching for the solution, we find a technique that the IntelliJ tool could give
us branch coverage, but still, we didn't have condition coverage. So we switched to Eclipse IDE and used EclEmma plugin
for coverage. Again, the result was the same and this tool also didn't show us condition coverage. After that, we decided to
return to IntelliJ IDE since our team has more experience in working with that, and we used the method coverage metric instead.
...
2. By trying to reach 100% statement/branches coverage, you may find new bugs in the code. As we find a new bug in Range class for CombineIgnoringNaN method. When we tried to combine the range with upper bound NaN values, the result wasn't as we expected. (You can see [aCombinationWithUpperBondNaNValue test CombineIgnoringNaNMethodTest file](./test/org/jfree/data/range/CombineIgnoringNaNMethodTest.java) for more information)
...
1. It may take a while for you to set up the tool and extract the required metrics from that. Especially, if it's your first time. But after that, you can work with them more straightforwardly.
2. The IntelliJ built-in coverage plugin, EclEmma, and JaCoCo tools don't have Condition Coverage metric.
3. You may end up with some dummy test cases only to reach 100% coverage, like testing getters/setters.

We used 3 metrics for our coverage report: Statement Coverage, Branch Coverage, and Method Coverage. Although what we said earlier is also true for these metrics, if we want to speak individually, we can say:
...
2. This is also true for the loop ending condition. This metric doesn't show if that has actually happened or not.

Pros of branch coverage:
1. The branches are the things that make your code more complicated since they don't follow a simple path anymore, and you are dealing with a tree. This metric helps you to make sure you've reached every node of that tree with your test cases.
2. This metric helps to measure which decision outcomes have been tested and which have not.

Cons of branch coverage:
1. In some conditions, you may have more than one term combined with &&/||/!. So it's complicated to figure out you have covered all of them or not. Although, IntelliJ has a tool that can show you the number of hits per term and that helped us.
...
Also, setting up the coverage tool and finding a tool that supports condition coverage took us a lot of time.
